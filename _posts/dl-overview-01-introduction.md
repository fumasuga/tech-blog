...
ディープラーニングのモデルは、**ニューロン（ユニット）** が**層**をなし、層と層の間で**重み付きの線形変換と非線形変換（活性化関数）** を繰り返すことで構成されます。ここでは、**ニューロン・層・活性化関数・損失関数・最適化**の5つを押さえます。

### 1. ニューロン（ユニット）

1つの**ニューロン**は、次のように振る舞います。

1. **複数の入力** \( x_1, x_2, \ldots, x_n \) を受け取る。
2. それぞれに**重み** \( w_1, w_2, \ldots, w_n \) をかけて足し、**バイアス** \( b \) を加える。つまり \( z = w_1 x_1 + w_2 x_2 + \cdots + w_n x_n + b \)（**線形結合**）。
3. \( z \) を**活性化関数** \( \sigma \) に通し、**出力** \( a = \sigma(z) \) を得る。
...
各層では、前の層の出力 \( \boldsymbol{x} \) に対して、**重み行列 \( \boldsymbol{W} \) とバイアス \( \boldsymbol{b} \) で線形変換** \( \boldsymbol{z} = \boldsymbol{W}\boldsymbol{x} + \boldsymbol{b} \) を行い、その後**活性化関数を要素ごとに適用**して \( \boldsymbol{a} = \sigma(\boldsymbol{z}) \) を次の層に渡します。この「線形 → 非線形」のブロックを何度も重ねることで、複雑な非線形関数を近似していきます。
...
- **回帰**: **二乗誤差（MSE）** \( \frac{1}{n}\sum_i (y_i - \hat{y}_i)^2 \) がよく使われます。予測 \( \hat{y}_i \) と正解 \( y_i \) の差の二乗の平均です。
...
1. **勾配降下法**: 損失 \( L \) をパラメータ \( \theta \) で偏微分した**勾配 \( \nabla_\theta L \)** を求め、「勾配の逆方向」にパラメータを少し動かします。\( \theta \leftarrow \theta - \eta \nabla_\theta L \)（\( \eta \) は**学習率**）。これを繰り返すことで、損失が減少していきます。
...
次の記事では、**ディープラーニングの要素技術**として、畳み込みニューラルネットワーク（CNN）・リカレントニューラルネットワーク（RNN）・Transformer・正則化・バッチ正規化・代表的なオプティマイザとフレームワークについて、役割と仕組みを詳しく解説します。
