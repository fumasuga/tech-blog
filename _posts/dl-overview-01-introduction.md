...
1. **順伝播**
入力データをネットワークに通し、層ごとに線形変換と活性化関数を適用して、**出力（予測）** を得る。
2. **損失の計算**
予測と正解を比較し、**損失関数**の値を計算する。
3. **逆伝播**
**バックプロパゲーション**で、損失に対する各パラメータの**勾配**を計算する。
4. **パラメータ更新**
勾配と**オプティマイザ**（SGD、Adamなど）に従い、**学習率**を考慮してパラメータを更新する。
