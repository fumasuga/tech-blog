...
機械学習では、**どのモデルを選ぶか**と**そのモデルがどれだけ良いか**を客観的に判断する必要があります。そのために、**データの扱い方**（訓練・検証・テストの分割、交差検証）と**評価指標**（予測誤差、正解率・適合率・再現率・F値、ROC曲線とAUC）、さらに**モデルの選択と情報量基準**（AIC、BIC、交差検証によるモデル選択）を押さえておくことが重要です。
...
2.$i = 1, …, K$について、第$i$フォールドを検証、残り$K-1$個を訓練としてモデルを学習し、検証データでスコアを計算する。
...
回帰では、**予測値**$\hat{y}$（モデルが出力した値）と**実測値**$y$（正解）の**誤差**を、何らかの形で1つの数にまとめた指標を使います。以下、$n$はデータ数、$y_i$は$i$番目の実測値、$\hat{y}_i$はその予測値とします。
...
**言葉で言うと**：「各データで（予測−実測）を計算し、それを2乗してから全部足し、データ数$n$で割ったもの」。大きな誤差ほど2乗で効くため強くペナルティがかかり、数学的にも扱いやすいので、回帰の損失関数としてよく使われます。単位は元の$y$の**二乗**になるため、解釈時には次の RMSE に変換することが多いです。
...
**言葉で言うと**：「各データで|予測−実測|を計算し、その平均を取ったもの」。外れ値の影響をMSEより受けにくく、「平均してどれだけずれているか」を直感的に解釈しやすいです。単位は$y$と同じです。
...
$\bar{y}$は実測値の平均です。**「実測値のばらつき」のうち、モデルで説明できた割合**を0〜1付近の数で表したものです。1に近いほど良く、0は「いつも平均を予測しているのと同じ」、負のときは平均より悪い予測をしていることを意味します。高校で習う「相関」や「決定係数」の考え方に近いです。スケールに依存しないため、異なるデータセット間のモデル比較に便利です。
...
**言葉で言うと**：AICは「**データへの当てはまりの良さ**（第一項：小さいほど良い）＋**モデルの複雑さへのペナルティ**（第二項：パラメータ数$k$が増えると大きくなる）」の和。$\ell(\hat{\theta})$は推定したパラメータでの対数尤度（当てはまりの良さ）、$k$はパラメータの数です。**AICが小さいモデルほど良い**と解釈します。複雑なモデルは当てはまりは良くなるが、その分ペナルティで不利になり、バランスの取れたモデルが選ばれやすくなります。統計モデル（回帰、時系列のARモデルなど）の**モデル選択**で広く使われます。
...
**言葉で言うと**：BICもAICと同様「当てはまり＋複雑さのペナルティ」だが、ペナルティが**データ数$n$の対数**に比例する（$n$が大きいと$\ln n > 2$なので、AICより**パラメータ数に厳しめ**）。**BICが小さいモデルほど良い**と解釈し、AICよりシンプルなモデルを選びがちです。時系列の次数選択（ARの$p$など）や、混合数やクラスタ数の選択にも使われます。
