...
教師なし学習（Unsupervised Learning）は、**正解ラベルなしのデータ**から、データの構造やパターン、グループ分けを自動的に発見する機械学習の手法です。ラベルを付けるコストが高い場合や、まずデータの傾向を把握したい場合に有効です。
この記事では、教師なし学習の基本に加え、**階層なしクラスタリング・階層ありクラスタリング・主成分分析・協調フィルタリング・トピックモデル**を、定式化・アルゴリズム・メリット・デメリットまで詳しく解説します。

### この記事でわかること

- 教師なし学習の目的（構造の発見・要約）と、教師あり学習との違い
- 階層なしクラスタリング（K-means、DBSCAN、GMM）と
  階層ありクラスタリング（凝集型・リンケージ）の考え方と使い分け
...
目的は**クラスタ内平方和（Within-Cluster Sum of Squares, WCSS）**、すなわち「各データと、そのデータが属するクラスタのセントロイドとの距離の二乗和」を最小にすることです。各データを最も近いセントロイドに割り当て、各クラスタ内のデータの平均を新たなセントロイドとして更新する。この一連の操作を、割り当てが変わらなくなるまで繰り返します。

#### アルゴリズムの手順

（記号の意味：$\mu_k$＝第$k$番目のクラスタの**中心（セントロイド）** の座標、$x_i$＝$i$番目のデータの座標、$C_k$＝$k$番目のクラスタに属するデータの集合、$|C_k|$＝その個数。）

1. **初期化**
K個のセントロイド$\mu_1, …, \mu_K$を、ランダムにK個のデータ点に置く、または k-means++ などで分散させる。
2. **割り当て**
各データ$x_i$を、**いまのセントロイドのうち最も近いもの**$\mu_k$に割り当てる（距離は普通、2点間の直線距離＝ユークリッド距離）。
3. **更新**
各クラスタ$C_k$について、**そのクラスタに属する点の座標の平均**を新セントロイドにする。
式で書くと
※「$C_k$に属する点を全部足して、個数で割る」
...
- **$\varepsilon$（イプシロン、eps）**: **「近い」とみなす半径**。この半径以内の点が「近傍」。小さいと細かく分かれ、大きいとクラスタがまとまりやすくノイズが減る。
...
**言葉で言うと**：「データは、$K$個の**山（ガウス分布）**のどれかからランダムに生じている」と仮定する。各山には**中心**$\mu_k$と**広がり（分散・共分散）**$\Sigma_k$があり、$k$番目の山が選ばれる**重み（混合比率）**が$\pi_k$（全部足すと1）。データ$x$が観測されたとき、**「そのデータがクラスタ$k$から来た確率」**$P(\text{クラスタ} k \mid x)$が所属確率として得られる。

式では、密度を以下と表す（$\mathcal{N}$はガウス分布。各山の密度を重み付きで足し合わせたもの）。
...
**データ**は行列$X$で表す（行＝1サンプル、列＝各特徴量）。各列の平均を0にしておく（**中心化**）。

- **第1主成分**: データの**ばらつきが最も大きい方向**を表す**単位ベクトル**$w_1$（長さ1のベクトル）。「その方向へデータを射影したときの分散」$\text{Var}(X w_1)$を最大にする$w_1$を求める（$\|w_1\| = 1$ は「長さ1」の条件）。
- **第2主成分以降**: 第1主成分と**無相関**（直交する）という条件の下で、同様に「分散が最大になる方向」を順に求める。第3、第4…と続ける。

### 固有値分解との関係

中心化したデータの**共分散行列**$\Sigma$（特徴量どうしの「一緒にばらつく度合い」を並べた行列）を考えると、**主成分の方向は$\Sigma$の固有ベクトル**、**各主成分の分散は対応する固有値**に一致します。高校で習う「固有値・固有ベクトル」の考え方で、分散が大きい順の方向が自動的に得られます。固有値が大きい順に並べた固有ベクトルが、第1、第2、…主成分です。

### 変換と再構成

- **射影（次元削減）**: データ$x$に行列$W$（主成分ベクトルを並べたもの）をかけて $z = W^\top x$ とすると、**主成分の座標**$z$が得られる。上位$k$個の主成分だけ使えば、$k$次元の表現になる（元より次元が減る）。
- **再構成**: 削減した$z_k$から、$\hat{x} = W_k z_k$ で**元の空間に戻す**と、捨てた主成分の分だけ誤差が出る。その誤差は「捨てた主成分の分散の和」に等しい（分散が小さい主成分を捨てるほど誤差は小さい）。
...
**ユーザー×アイテム**の表（行列）を$R$とする。$R_{ui}$は「ユーザー$u$がアイテム$i$に付けた評価」（またはクリックしたかなど）を表す。多くのマスは**空欄**（まだ評価していない・まだ見ていない）で、**その空欄を埋める＝予測する**ことが目的になる。
