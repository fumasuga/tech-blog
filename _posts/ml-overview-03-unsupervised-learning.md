...
1. **初期化**: K個のセントロイド$\mu_1, …, \mu_K$を、ランダムにK個のデータ点に置く、または k-means++ などで分散させる。
2. **割り当て**: 各データ$x_i$を、**いまのセントロイドのうち最も近いもの**$\mu_k$に割り当てる（距離は普通、2点間の直線距離＝ユークリッド距離）。
3. **更新**: 各クラスタ$C_k$について、**そのクラスタに属する点の座標の平均**を新セントロイドにする。式で書くと $\mu_k = \frac{1}{|C_k|} \sum_{x \in C_k} x$（「$C_k$に属する点を全部足して、個数で割る」）。
...
式では、密度を $p(x) = \sum_{k=1}^K \pi_k \, \mathcal{N}(x \mid \mu_k, \Sigma_k)$ と表す（$\mathcal{N}$はガウス分布。各山の密度を重み付きで足し合わせたもの）。
